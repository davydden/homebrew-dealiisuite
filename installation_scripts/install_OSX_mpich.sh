...
if [ "$useMKL" = true ] ; then
  ...
  echo -e "\033[1;37m. /opt/intel/mkl/bin/mklvars.sh intel64 mod lp64\033[0m"
...
if [ "$useMKL" = true ] ; then
  # for Single Dynamic Library : mkl_rt
  # export MKL_INTERFACE_LAYER=MKL_INTERFACE_LP64
  ...
  # Intel MKL Link Line Advisor v4.6
  # MKL 11.3.2, OSX, Clang, Intel 64, Dynamic, LP64, Sequential, ScaLAPACK, BLACS
...
if [ "$useMKL" = false ]; then
  brew install scalapack --env=std --without-check
fi

brew install mumps --env=std --with-mkl && \
brew test mumps && \
brew install petsc --env=std --with-mkl && \
brew test petsc && \
brew install arpack --with-mpi --env=std && \
brew test arpack && \
brew install slepc --env=std --with-mkl && \
brew test slepc && \
brew install p4est --env=std && \
brew install suite-sparse --env=std && \
brew install trilinos --env=std --with-mkl && \
brew test trilinos && \
...
brew install dealii --env=std --with-mkl && \
brew test dealii
